...
# ---------- places ----------
ZONES=set(); place={}   # name -> dict(kind, zone, level, stage)
    ...
    for sd in entry['sources']:
        ...
        if key in dd:
            ...
            if sd['note'] and sd['note'] not in o['note']:
                ...
                if len(po)==2 and len(ps)==2 and po[0]==ps[0]: o['note']=po[0]+': '+po[1]+', '+ps[1]
    ...
    if page:
        ...
        for t in ps.select('#mw-content-text table'):
            ...
            for rr in rows[1:]:
                ...
                entry['sources'].append(dict(k=k,name=mob,t=mtitle,img=mimg,loc=plc,xy=xy if k in ('world','fate','leve','questmob') else None,lv=lv,lvMax=lvmax,note=note,rank=rank,rec=(mob==hinted)))
